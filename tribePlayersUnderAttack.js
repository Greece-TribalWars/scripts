...
var scriptConfig = {
	...
	allowedMarkets: [], // allowed on all markets
	allowedScreens: [], // allowed on all game screens
	allowedModes: [], // allowed on all game modes
	isDebug: DEBUG, // enable/disable debug mode
	enableCountApi: true, // enable/disable hit counter
};
...
$.getScript('https://twscripts.dev/scripts/twSDK.js', function () {
		...
		if (playersToFetch.length) {
			...
			twSDK.getAll(
				...
				function (index, data) {
					...
					const villagesTableRows = jQuery(htmlDoc).find(`.table-responsive table tr`).not(':first');
					...
					if (villagesTableRows && villagesTableRows.length) {
						villagesTableRows.each(function () {
							try {
								...
								const incomingsCount = parseInt(_this.find('td:last').text().trim());
								if (incomingsCount > 0) {
									...
									const currentVillageName = _this.find('td:first').text().trim();
									...
									_this
										...
										.each(function () {
											const unitAmount =
												jQuery(this).text().trim() !== '?' ? jQuery(this).text().trim() : 0;
					...
					let totalIncs = villageIncs.map((incoming) => incoming.incsCount);
				...
				function () {
					...
					try {
						...
						const customStyle = `
							...
							.ra-player-incomings h3 { user-select: none; font-weight: normal; margin: 0; padding: 5px; font-size: 14px; background-color: #c1a264 !important; background-image: url(/graphic/screen/tableheader_bg3.png); background-repeat: repeat-x; position: relative; cursor: pointer; }
				...
				twSDK.getAll(
					...
					function (index, data) {
						...
						let incomingsCount = jQuery(htmlDoc)
							...
							.text()
							.trim();
						incomingsCount = parseInt(incomingsCount.substring(1, incomingsCount.length - 1));
					...
					function () {
						...
						try {
							...
							const tribeMembersList = [...playersWhoHaveShared, ...playersWhoHaveNotShared];
	...
	function buildPlayerIncomingsTable(playerIncomings) {
		...
		playerIncomings.forEach((player) => {
			const { id, incomings, name, totalIncomingsNumber, totalPlayerVillages, villagesUnderAttack } = player;
			...
			if (incomings.length) {
				...
				const incomingsPerTotalVillageRatio = parseFloat(totalIncomingsNumber / totalPlayerVillages).toFixed(2);

				playerIncomingsList += `
					<div class='ra-player-incomings'>
						<h3 class="ra-player-incomings-toggle">
							<a href='/game.php?screen=info_player&id=${id}' target='_blank' rel='noopener noreferrer'>
								${name} (${twSDK.formatAsNumber(totalPlayerVillages)})
							</a> 
							- ${twSDK.tt('Incomings')}: <b>${totalIncs}</b> - ${twSDK.tt('Villages under attack:')} <b>${twSDK.formatAsNumber(
	...
	async function getTribeMembersList() {
		let troopsMemberPage = '/game.php?village=' + game_data.village.id + '&screen=ally&mode=members_defense';
	...
	async function getTribeMembersListNotLeader() {
		let troopsMemberPage = '/game.php?village=' + game_data.village.id + '&screen=ally&mode=members_troops';
		if (game_data.player.sitter != '0') {
			troopsMemberPage += '&t=' + game_data.player.id;
		}
	...
	function togglePlayerExpandableWidget() {
		jQuery('.ra-player-incomings-toggle').on('click', function () {
			jQuery(this).parent().find('.ra-player-incomings-table').slideToggle(50);
